...
/** S-01 初期画面モック */
export default function InitScreenMock() {
  // 表示パターン切り替え用
  const [pattern, setPattern] = useState<"initial" | "filled" | "error">(
    "initial"
  );
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 6 }}>
      ...
      <Box sx={{ display: "flex", gap: 1, mb: 3, justifyContent: "center" }}>
        <Button
          size="small"
          variant={pattern === "initial" ? "contained" : "outlined"}
          onClick={() => setPattern("initial")}
        >
          初期
        </Button>
        <Button
          size="small"
          variant={pattern === "filled" ? "contained" : "outlined"}
          onClick={() => setPattern("filled")}
        >
          入力済み
        </Button>
        <Button
          size="small"
          variant={pattern === "error" ? "contained" : "outlined"}
